...
<head>
  ...
  <title>SISDEV - Barra do Rio</title>
...
<body>
  ...
  <script type="text/babel">
    ...
    const Package = () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="16.5" y1="9.4" x2="7.5" y2="4.21"/><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>;
    ...
    const Map = () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="1 6 1 22 8 18 16 22 23 18 23 2 16 6 8 2 1 6"/><line x1="8" y1="2" x2="8" y2="18"/><line x1="16" y1="6" x2="16" y2="22"/></svg>;
    ...
    const SISDEV = () => {
      ...
      const agendamentos = [
        ...
        { id: 4, container: 'CMAU7778899', armador: 'CMA CGM', importador: 'Bretzke Logística', data: '2025-10-12', status: 'atrasado' }
      ];
          ...
          <div className="bg-white border-b border-gray-200 overflow-x-auto">
            <div className="max-w-7xl mx-auto">
              <nav className="flex gap-0 min-w-max">
                <button onClick={() => setActiveTab('dashboard')} className={`flex items-center gap-2 px-4 md:px-6 py-4 font-medium transition-colors border-b-2 whitespace-nowrap ${activeTab === 'dashboard' ? '' : 'text-gray-600 border-transparent'}`} style={activeTab === 'dashboard' ? {color: '#F39200', borderColor: '#F39200'} : {}}>
                  <BarChart3 />Dashboard
                </button>
                <button onClick={() => setActiveTab('agendamentos')} className={`flex items-center gap-2 px-4 md:px-6 py-4 font-medium transition-colors border-b-2 whitespace-nowrap ${activeTab === 'agendamentos' ? '' : 'text-gray-600 border-transparent'}`} style={activeTab === 'agendamentos' ? {color: '#F39200', borderColor: '#F39200'} : {}}>
                  <Calendar />Agendamentos
                </button>
                <button onClick={() => setActiveTab('mapa')} className={`flex items-center gap-2 px-4 md:px-6 py-4 font-medium transition-colors border-b-2 whitespace-nowrap ${activeTab === 'mapa' ? '' : 'text-gray-600 border-transparent'}`} style={activeTab === 'mapa' ? {color: '#F39200', borderColor: '#F39200'} : {}}>
                  <Map />Mapa
                </button>
                <button onClick={() => setActiveTab('marketplace')} className={`flex items-center gap-2 px-4 md:px-6 py-4 font-medium transition-colors border-b-2 whitespace-nowrap ${activeTab === 'marketplace' ? '' : 'text-gray-600 border-transparent'}`} style={activeTab === 'marketplace' ? {color: '#F39200', borderColor: '#F39200'} : {}}>
                  <Package />Marketplace
                </button>
                <button onClick={() => setActiveTab('relatorios')} className={`flex items-center gap-2 px-4 md:px-6 py-4 font-medium transition-colors border-b-2 whitespace-nowrap ${activeTab === 'relatorios' ? '' : 'text-gray-600 border-transparent'}`} style={activeTab === 'relatorios' ? {color: '#F39200', borderColor: '#F39200'} : {}}>
                ...
                <div className="bg-white p-4 md:p-6 rounded-lg shadow-sm">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
                    <div className="text-center">
                      <p className="text-xs md:text-sm text-gray-500 mb-2">Total Depots</p>
                      <p className="text-2xl md:text-4xl font-light text-gray-800">18</p>
                    </div>
                    <div className="text-center md:border-l border-gray-200">
                      <p className="text-xs md:text-sm text-gray-500 mb-2">Disponíveis</p>
                      <p className="text-2xl md:text-4xl font-light text-green-600">8</p>
                    </div>
                    <div className="text-center md:border-l border-gray-200">
                      <p className="text-xs md:text-sm text-gray-500 mb-2">Devoluções Hoje</p>
                      <p className="text-2xl md:text-4xl font-light" style={{color: '#003B71'}}>47</p>
                    </div>
                    <div className="text-center md:border-l border-gray-200">
                      <p className="text-xs md:text-sm text-gray-500 mb-2">Tempo Médio</p>
                      <p className="text-2xl md:text-4xl font-light" style={{color: '#F39200'}}>42min</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-4 md:p-6 rounded-lg shadow-sm">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      <select value={filtroArmador} onChange={(e) => setFiltroArmador(e.target.value)} className="text-sm border border-gray-300 rounded px-3 py-2">
                        <option value="todos">Todos Armadores</option>
                        <option value="Maersk">Maersk</option>
                        <option value="MSC">MSC</option>
                        <option value="Hamburg Sud">Hamburg Sud</option>
                      </select>
                      <select value={filtroStatus} onChange={(e) => setFiltroStatus(e.target.value)} className="text-sm border border-gray-300 rounded px-3 py-2">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                    {depotsFiltrados.map(depot => (
                      <div key={depot.id} className="flex items-center justify-between p-4 border border-gray-100 hover:shadow-md transition-shadow rounded">
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          <div className={`w-3 h-3 rounded-full flex-shrink-0 ${getStatusColor(depot.status)}`}></div>
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-gray-800 truncate">{depot.nome}</p>
                            <p className="text-xs text-gray-500 truncate">Cordeiros</p>
                    <div className="flex justify-between mt-1 md:mt-2 pt-1 md:pt-2 border-t">
                      <span className="text-xs">Cap:</span>
                      <span className="text-xs md:text-sm font-bold text-green-600">85%</span>
                    </div>
                  </div>

                  <div className="absolute bottom-20 md:bottom-32 left-8 md:left-32 bg-white rounded-lg shadow-md p-2 md:p-3 border-l-4 border-red-500 w-36 md:w-48">
                    <div className="flex items-center justify-between mb-1 md:mb-2">
                      <MapPin />
                      <div className="w-2 h-2 md:w-3 md:h-3 rounded-full bg-red-500"></div>
                    </div>
                    <p className="text-xs md:text-sm font-bold truncate">Depot MSC</p>
                    <p className="text-xs text-gray-500 truncate">Porto</p>
                    <div className="flex justify-between mt-1 md:mt-2 pt-1 md:pt-2 border-t">
                      <span className="text-xs">Cap:</span>
                      <span className="text-xs md:text-sm font-bold text-red-600">95%</span>
                    </div>
                  </div>

                  <div className="absolute top-20 md:top-32 right-4 md:right-16 bg-white rounded-lg shadow-md p-2 md:p-3 border-l-4 border-green-500 w-36 md:w-48">
                    <div className="flex items-center justify-between mb-1 md:mb-2">
                      <MapPin />
                      <div className="w-2 h-2 md:w-3 md:h-3 rounded-full bg-green-500"></div>
                    </div>
                    <p className="text-xs md:text-sm font-bold truncate">Hamburg Sud</p>
                    <p className="text-xs text-gray-500 truncate">Centro</p>
                    <div className="flex justify-between mt-1 md:mt-2 pt-1 md:pt-2 border-t">
                      <span className="text-xs">Cap:</span>
                      <span className="text-xs md:text-sm font-bold text-green-600">45%</span>
                    </div>
                  </div>

                  <div className="absolute bottom-20 md:bottom-32 right-8 md:right-32 bg-white rounded-lg shadow-md p-2 md:p-3 border-l-4 border-red-500 w-36 md:w-48">
                    <div className="flex items-center justify-between mb-1 md:mb-2">
                      <MapPin />
                      <div className="w-2 h-2 md:w-3 md:h-3 rounded-full bg-red-500"></div>
                    </div>
                    <p className="text-xs md:text-sm font-bold truncate">Depot Cosco</p>
                    <p className="text-xs text-gray-500 truncate">Gravatá</p>
                    <div className="flex justify-between mt-1 md:mt-2 pt-1 md:pt-2 border-t">
                      <span className="text-xs">Cap:</span>
                      <span className="text-xs md:text-sm font-bold text-red-600">100%</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-4 md:p-6 rounded-lg shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-8">
                  <div className="border-l-4 pl-4" style={{borderColor: '#F39200'}}>
                    ...
                    <p className="text-2xl md:text-3xl font-light text-gray-800">R$ 680K</p>
                  ...
                  <div className="border-l-4 pl-4" style={{borderColor: '#003B71'}}>
                    ...
                    <p className="text-2xl md:text-3xl font-light text-gray-800">7.891</p>
                    <p className="text-xs text-gray-500 mt-1">Últimos 6 meses</p>
                  </div>
                  <div className="border-l-4 border-green-500 pl-4">
                    <p className="text-sm text-gray-600 mb-1">Tempo Médio</p>
                    <p className="text-2xl md:text-3xl font-light text-gray-800">42min</p>
                    ...
                    {[
                      ...
                      <div key={idx}>
                        ...
                        <div className="w-full bg-gray-100 h-2 rounded overflow-hidden">
                          <div className="h-2 rounded transition-all duration-300" style={{backgroundColor: '#003B71', width: `${(item.tempo / 70) * 100}%`}}></div>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          {showNovoAgendamento && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-lg max-w-2xl w-full max-h-screen overflow-y-auto">
                <div className="p-4 md:p-6 border-b border-gray-200 flex items-center justify-between" style={{backgroundColor: '#003B71'}}>
                  <h3 className="text-lg md:text-xl font-medium text-white">Novo Agendamento</h3>
                ...
                <div className="p-4 md:p-6 space-y-4">
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="Nome da empresa" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" />
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Depot Sugerido</label>
                    <div className="p-4 border-2 rounded-lg" style={{borderColor: '#F39200', backgroundColor: '#FFF4E6'}}>
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-medium text-gray-800 text-sm md:text-base">Depot Hapag-Lloyd Itajaí</p>
                          <p className="text-xs md:text-sm text-gray-600">São João - Itajaí</p>
                          <p className="text-xs text-gray-500 mt-1">Cap: 22% • Espera: 10 min</p>
                        </div>
                        <div className="w-3 h-3 rounded-full bg-green-500 flex-shrink-0"></div>
                      </div>
                      <p className="text-xs mt-2 text-gray-600">✓ Melhor opção disponível</p>
                    </div>
                  </div>
                </div>

                <div className="p-4 md:p-6 border-t border-gray-200 flex flex-col sm:flex-row gap-3 justify-end">
                  <button onClick={() => setShowNovoAgendamento(false)} className="w-full sm:w-auto px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                    Cancelar
                  </button>
                  <button onClick={() => setShowNovoAgendamento(false)} className="w-full sm:w-auto px-6 py-2 text-white rounded-lg hover:opacity-90" style={{backgroundColor: '#F39200'}}>
                    Confirmar
                  </button>
                </div>
              </div>
    ...
    ReactDOM.render(<SISDEV />, document.getElementById('root'));
  </script>
</body>
</html>ate">{depot.armador} • {depot.localizacao}</p>
                          </div>
                        </div>
                        <div className="text-right ml-4 flex-shrink-0">
                          <p className="text-sm text-gray-700 font-medium">{depot.capacidade}%</p>
                  ...
                  <div className="bg-white p-4 md:p-6 rounded-lg shadow-sm">
                    ...
                    <div className="space-y-3">
                      {alertas.map((alerta, idx) => (
                        <div key={idx} className={`p-3 rounded ${getAlertStyle(alerta.tipo)}`}>
                          <div className="flex items-start gap-2">
                            <div className={`flex-shrink-0 ${getAlertIcon(alerta.tipo)}`}><AlertCircle /></div>
                  ...
                  <div className="bg-white p-4 md:p-6 rounded-lg shadow-sm">
                    ...
                    <div className="space-y-4">
                      <div className="border-l-4 pl-4" style={{borderColor: '#F39200'}}>
                        ...
                        <p className="text-2xl md:text-3xl font-light text-gray-800 mt-1">R$ 127.450</p>
                      </div>
                      <div className="border-l-4 pl-4" style={{borderColor: '#003B71'}}>
                        <p className="text-sm text-gray-600">Tempo Reduzido</p>
                        <p className="text-2xl md:text-3xl font-light text-gray-800 mt-1">-38%</p>
                      </div>
                      <div className="border-l-4 border-green-500 pl-4">
                        <p className="text-sm text-gray-600">Devoluções</p>
                        <p className="text-2xl md:text-3xl font-light text-gray-800 mt-1">1.243</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {activeTab === 'agendamentos' && (
              <div className="bg-white p-4 md:p-6 rounded-lg shadow-sm">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                  <h3 className="text-lg font-medium text-gray-800">Agendamentos</h3>
                  <button onClick={() => setShowNovoAgendamento(true)} className="flex items-center justify-center gap-2 text-white px-4 md:px-6 py-2 md:py-3 text-sm font-medium rounded" style={{backgroundColor: '#F39200'}}>
                    <Calendar size={18} />
                    Novo Agendamento
                  </button>
                </div>

                <div className="mb-6 p-4 border-l-4 rounded" style={{backgroundColor: '#FFF4E6', borderColor: '#F39200'}}>
                  <p className="text-sm text-gray-800 font-medium">💡 Agende sua devolução!</p>
                </div>

                <div className="mb-4">
                  <div className="relative">
                    <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"><Search /></div>
                    <input type="text" placeholder="Buscar..." value={busca} onChange={(e) => setBusca(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg" />
                  </div>
                </div>
                  ...
                  <table className="w-full min-w-max">
                    <thead>
                      <tr className="border-b-2" style={{borderColor: '#003B71'}}>
                        <th className="p-3 text-left text-sm font-medium whitespace-nowrap">Container</th>
                        <th className="p-3 text-left text-sm font-medium whitespace-nowrap">Armador</th>
                        <th className="p-3 text-left text-sm font-medium whitespace-nowrap">Importador</th>
                        <th className="p-3 text-center text-sm font-medium whitespace-nowrap">Data</th>
                        <th className="p-3 text-center text-sm font-medium whitespace-nowrap">Status</th>
                      </tr>
                    </thead>
                    <tbody>
                      {agendamentosFiltrados.map((ag) => (
                        <tr key={ag.id} className="border-b border-gray-100">
                          <td className="p-3 text-sm font-medium whitespace-nowrap">{ag.container}</td>
                          <td className="p-3 text-sm whitespace-nowrap">{ag.armador}</td>
                          <td className="p-3 text-sm whitespace-nowrap">{ag.importador}</td>
                          <td className="p-3 text-sm text-center whitespace-nowrap">{ag.data}</td>
                          <td className="p-3 text-center whitespace-nowrap">
                            {ag.status === 'confirmado' && <span className="text-xs bg-green-500 text-white px-3 py-1 rounded-full inline-block">Confirmado</span>}
                            {ag.status === 'aguardando' && <span className="text-xs bg-yellow-500 text-white px-3 py-1 rounded-full inline-block">Aguardando</span>}
                            {ag.status === 'atrasado' && <span className="text-xs bg-red-500 text-white px-3 py-1 rounded-full inline-block">Atrasado</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            {activeTab === 'mapa' && (
              <div className="bg-white p-4 md:p-6 rounded-lg shadow-sm">
                ...
                <div className="relative bg-gradient-to-br from-blue-50 to-blue-100 p-4 md:p-8 rounded-lg overflow-hidden" style={{minHeight: '500px', height: '600px'}}>
                  <div className="absolute top-4 left-4 bg-white p-2 md:p-3 rounded shadow-sm border-2" style={{borderColor: '#003B71'}}>
                    <p className="text-xs font-bold">ITAJAÍ</p>
                  </div>
                  <div className="absolute top-4 right-4 bg-white p-2 md:p-3 rounded shadow-sm border-2" style={{borderColor: '#003B71'}}>
                    <p className="text-xs font-bold">NAVEGANTES</p>
                  </div>
                  <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-blue-900 text-white px-3 md:px-4 py-2 rounded-lg text-center">
                    <p className="text-xs font-bold whitespace-nowrap">⚓ PORTO</p>
                  </div>

                  <div className="absolute top-20 md:top-32 left-4 md:left-16 bg-white rounded-lg shadow-md p-2 md:p-3 border-l-4 border-green-500 w-36 md:w-48">
                    <div className="flex items-center justify-between mb-1 md:mb-2">
                      <MapPin />
                      <div className="w-2 h-2 md:w-3 md:h-3 rounded-full bg-green-500"></div>
                    </div>
                    <p className="text-xs md:text-sm font-bold truncate">Depot Maersk</p>
                    <p className="text-xs text-gray-500 trunc
